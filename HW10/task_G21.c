...
int main(int argc, char **argv)
{
    ...
    if(star_count < accum) fprintf(fp_out, "NO");
    else
    {
        for(int i = 1; i <= row; i++)
        {
...
void print_star(int row_length, int star_quantity, FILE* fp_out)
{
    ...
    for(int j = 0; j < (row_length - star_length)/2; j++) fprintf(fp_out, " ");
    for(int j = 0; j < star_quantity; j++) fprintf(fp_out, "* ");
    fprintf(fp_out, "\n");
}
